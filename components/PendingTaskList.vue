<template>
  ...
  >
    ...
    <v-list color="yellow lighten-4 mt-3">
      <draggable ghostClass="on-drag" @end="updateMainId($event)" tag="v-col" animation="300" group="items">
        <v-col
              ...
              <div v-show="item.show" class="pb-2">
                ...
                <draggable v-if="item.detailTasks" animation="300" tag="v-col">
                  <v-col
                    ...
                    class="pa-1"
                  >
                    <v-card color="yellow lighten-5" v-if="detailTask">
...
export default {
  ...
  async mounted() {
    ...
    const snapshot = await this.db.where('type', '==', 'pending').get();
    if (snapshot.empty) {
    ...
    snapshot.forEach(async (doc) => {
      ...
      const subCollection = await doc.ref.collection('detailTasks').where('type', '==', 'pending').get();
  ...
  methods: {
    ...
    updateMainId(event) {
      ...
      this.mainItems[event.oldIndex].id = event.newIndex
      this.mainItems[event.newIndex].id = event.oldIndex
    },
